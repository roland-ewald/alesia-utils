 ...
 *
 */
class JamesIIComparator(muSelector: MatchUpSelector[sessl.Simulator], teamsPerGame: Int) 
			extends TSComparator() {
	private[this] var aR: ActiveRanking = null 
    def register(aR: ActiveRanking) = this.aR = aR 
	
    val executor = new CachedExperimentExecutor("teamPerformances1.xml", "teamPerformances2.xml", "teamPerformances3.xml", "teamPerformances4.xml")
